...
from app.utils.common import get_db_session
from app.services import conversation_svc
from app.schemas.conversation_schm import ConversationCreate, Conversation
from app.schemas.message_schm import Message as SchmMessage
...
async def create_conversation(
    ...
    """
    return await conversation_svc.create_conversation(db, payload)


@router.get("/{conversation_id}")
async def get_conversation(
    conversation_id: UUID, db: AsyncSession = Depends(get_db_session)
) -> Conversation:
    """
    Get a conversation by ID along with its messages and message subprocesses.
    """
    conversation = await conversation_svc.fetch_conversation_with_messages(db, str(conversation_id))
...
async def delete_conversation(
    ...
    """
    did_delete = await conversation_svc.delete_conversation(db, str(conversation_id))
    if not did_delete:
        raise HTTPException(status_code=404, detail="Conversation not found")
    return
...
async def message_conversation(
    ...
    async def event_publisher():
        async with send_chan:
            ...
            try:
                async for message_obj in recv_chan:
                    if isinstance(message_obj, StreamedMessage):
                        ...
                        continue
                    yield SchmMessage.from_orm(message).json()
                await task
                if task.exception():
                    raise ValueError(
            ...
            db.add(message)
            await db.commit()
            final_message = await conversation_svc.fetch_message_with_sub_processes(db, message_id)
...
async def test_message_conversation(
    ...
    if final_message is not None:
        return SchmMessage.parse_raw(final_message)
    else:
